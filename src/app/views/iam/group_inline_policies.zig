...
const IamGroupInlinePolicyDocumentView = @import("group_inline_policy_document.zig");
const fetcher = @import("../../../sdk/credentials/fetcher.zig");
const Credentials = fetcher.Credentials;

const IamGroupInlinePoliciesView = @This();
pub const name: []const u8 = "IAM Group Inline Policies";
...
pub fn init(
    ...
    var view = IamGroupInlinePoliciesView{
        .fg_color = fg_color,
        .bg_color = bg_color,
        .list = .{
            .items = display,
...
pub fn breadcrumb(self: *IamGroupInlinePoliciesView) []const u8 {
    return self.breadcrumb_buf[0..self.breadcrumb_len];
}

pub fn deinit(self: *IamGroupInlinePoliciesView) void {
...
pub fn handleEvent(self: *IamGroupInlinePoliciesView, event: Event, vctx: ViewContext) !Action {
    switch (event) {
        .key => |k| switch (k) {
            .ctrl_c => return .quit,
            .char => |c| switch (c) {
            ...
            .enter => {
                ...
                const view = try IamGroupInlinePolicyDocumentView.init(
                    vctx.allocator,
                    self.io,
                    self.credentials,
                    self.group_name,
                ...
                return .{ .push = .{ .iam_group_inline_policy_document = view } };
            },
            .escape => return .pop,
            else => {},
        },
...
pub fn render(self: *IamGroupInlinePoliciesView, writer: *std.Io.Writer, size: Coord) !void {
