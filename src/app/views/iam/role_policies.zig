...
const IamRolePoliciesView = @This();
pub const name: []const u8 = "IAM Role Policies";
...
/// Owned deep copies of the attached policy name/arn pairs (parallel to `list.items`).
names: [][]u8,
arns: [][]u8,
io: std.Io,
...
pub fn init(
    ...
    const arns = try allocator.alloc([]u8, policy_arns.len);
    errdefer allocator.free(arns);
    var copied_arns: usize = 0;
    errdefer for (arns[0..copied_arns]) |a| allocator.free(a);
    for (policy_arns, 0..) |a, i| {
        arns[i] = try allocator.dupe(u8, a);
        copied_arns += 1;
    }
    ...
    const bc = std.fmt.bufPrint(&view.breadcrumb_buf, "{s} {s} Attached Policies", .{ parent_breadcrumb, constants.SEP_ARROW }) catch view.breadcrumb_buf[0..0];
    view.breadcrumb_len = bc.len;
    return view;
}

pub fn breadcrumb(self: *IamRolePoliciesView) []const u8 {
    return self.breadcrumb_buf[0..self.breadcrumb_len];
}

pub fn deinit(self: *IamRolePoliciesView) void {
    self.alloc.free(self.list.items);
    for (self.names) |n| self.alloc.free(n);
    self.alloc.free(self.names);
    for (self.arns) |a| self.alloc.free(a);
    self.alloc.free(self.arns);
}

// ─── Event handling ───────────────────────────────────────────────────────────

pub fn handleEvent(self: *IamRolePoliciesView, event: Event, vctx: ViewContext) !Action {
    switch (event) {
        .key => |k| switch (k) {
            .ctrl_c => return .quit,
            .char => |c| switch (c) {
            ...
            .enter => {
                ...
                const view = try IamPolicyView.init(
                    vctx.allocator,
                    self.io,
                    self.credentials,
                    self.names[self.list.selected],
                    self.arns[self.list.selected],
                    self.fg_color,
                    self.bg_color,
                    self.breadcrumb(),
                );
                return .{ .push = .{ .iam_policy = view } };
            },
            .escape => return .pop,
            else => {},
        },
...
pub fn render(self: *IamRolePoliciesView, writer: *std.Io.Writer, size: Coord) !void {
