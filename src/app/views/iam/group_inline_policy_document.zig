...
const IamGroupInlinePolicyDocumentView = @This();
pub const name: []const u8 = "IAM Group Inline Policy Document";

const Match = struct { line: usize, col: usize };

// ─── Background getGroupPolicy context ────────────────────────────────────────
...
fn fetchThread(ctx: *FetchCtx) void {
    ...
    const result = client.getGroupPolicy(.{ .group_name = ctx.group_name, .policy_name = ctx.policy_name }) catch |e| {
        ctx.err = e;
        return;
    };
    defer result.deinit();
...
pub fn init(
...
) !IamGroupInlinePolicyDocumentView {
    const policy_name_owned = try allocator.dupe(u8, policy_name);
    errdefer allocator.free(policy_name_owned);
    const ref_group_name = try allocator.dupe(u8, group_name);
    errdefer allocator.free(ref_group_name);
    ...
    var view = IamGroupInlinePolicyDocumentView{
        .fg_color = fg_color,
        .bg_color = bg_color,
        ...
        .io = io,
        .credentials = credentials,
        .refresh_group_name = ref_group_name,
...
pub fn breadcrumb(self: *IamGroupInlinePolicyDocumentView) []const u8 {
    return self.breadcrumb_buf[0..self.breadcrumb_len];
}

pub fn deinit(self: *IamGroupInlinePolicyDocumentView) void {
    ...
    self.alloc.free(self.refresh_group_name);
    self.alloc.free(self.refresh_policy_name);
}

fn effectiveFilter(self: *const IamGroupInlinePolicyDocumentView) []const u8 {
    if (self.live_filter.len > 0) return self.live_filter;
    return self.committed_filter orelse "";
}

pub fn setLiveFilter(self: *IamGroupInlinePolicyDocumentView, text: []const u8) void {
    self.live_filter = text;
}

pub fn commitFilter(self: *IamGroupInlinePolicyDocumentView, text: []const u8) void {
    if (self.committed_filter) |f| self.alloc.free(f);
    self.committed_filter = if (text.len > 0) self.alloc.dupe(u8, text) catch null else null;
    self.live_filter = "";
}

fn recomputeMatches(self: *IamGroupInlinePolicyDocumentView, query: []const u8) void {
...
fn refresh(self: *IamGroupInlinePolicyDocumentView) !void {
    ...
    const group_name_owned = try alloc.dupe(u8, self.refresh_group_name);
    errdefer alloc.free(group_name_owned);
...
pub fn handleEvent(self: *IamGroupInlinePolicyDocumentView, event: Event, _: ViewContext) !Action {
    switch (event) {
        .key => |k| switch (k) {
            .ctrl_c => return .quit,
            .char => |c| switch (c) {
...
pub fn render(self: *IamGroupInlinePolicyDocumentView, writer: *std.Io.Writer, size: Coord) !void {
...
fn writeStatus(self: *IamGroupInlinePolicyDocumentView, writer: *std.Io.Writer, inner_w: usize, data_rows: usize, msg: []const u8) !void {
    for (0..data_rows) |row| {
        try writer.writeAll(self.fg_color);
        try writer.writeAll(constants.VERTICAL);
        try writer.writeAll(terminal.RESET);
...
fn writeBottom(self: *IamGroupInlinePolicyDocumentView, writer: *std.Io.Writer, inner_w: usize) !void {
    try writer.writeAll(self.fg_color);
