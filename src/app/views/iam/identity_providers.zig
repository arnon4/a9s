...
const IamOidcProviderView = @import("oidc_provider.zig");
const IamSamlProviderView = @import("saml_provider.zig");

const IamIdentityProvidersView = @This();
pub const name: []const u8 = "Identity Providers";

// TYPE column: "OIDC" / "SAML" + padding.
// ACCOUNT column: 12-digit AWS account id + 2 padding.
// CREATED / EXPIRES columns: ISO-8601 date "2013-04-18" = 10 chars + 2 padding.
const TYPE_W: usize = 8;
const ACCOUNT_W: usize = 14;
const CREATED_W: usize = 14;
const EXPIRES_W: usize = 14;

pub const ProviderType = enum { oidc, saml };

pub const ProviderSortKey = enum { name, type, account, created };

const SortCtx = struct {
    items: []const ProviderItem,
    keys: []const ProviderSortKey,
...
fn compareField(a: ProviderItem, b: ProviderItem, key: ProviderSortKey) std.math.Order {
    return switch (key) {
        .name => std.mem.order(u8, a.name, b.name),
        .type => std.mem.order(u8, typeLabel(a.provider_type), typeLabel(b.provider_type)),
        .account => std.mem.order(u8, a.account_id, b.account_id),
        .created => std.mem.order(u8, a.create_date orelse "", b.create_date orelse ""),
    };
}

fn typeLabel(t: ProviderType) []const u8 {
    return switch (t) {
        .oidc => "OIDC",
        .saml => "SAML",
    };
...
const ProviderItem = struct {
    ...
    pub fn deinit(self: ProviderItem) void {
        self.allocator.free(self.name);
        self.allocator.free(self.arn);
        self.allocator.free(self.account_id);
        if (self.create_date) |d| self.allocator.free(d);
        if (self.valid_until) |v| self.allocator.free(v);
    }
};

const ProviderResolver = struct {
    item: ProviderItem,

    pub fn resolve(self: ProviderResolver, field: []const u8) filter_mod.FieldValue {
        if (std.mem.eql(u8, field, "name")) return .{ .string = self.item.name };
        if (std.mem.eql(u8, field, "arn")) return .{ .string = self.item.arn };
        if (std.mem.eql(u8, field, "type")) return .{ .string = typeLabel(self.item.provider_type) };
        if (std.mem.eql(u8, field, "account") or std.mem.eql(u8, field, "account_id")) return .{ .string = self.item.account_id };
        if (std.mem.eql(u8, field, "created") or std.mem.eql(u8, field, "create_date")) return .{ .string = self.item.create_date orelse "" };
        return .unknown;
    }
};

// Parse account id from an IAM provider ARN: arn:aws:iam::{account}:oidc-provider/{name} or :saml-provider/{name}
...
// Parse the provider's name from the last path segment of its ARN.
fn parseName(arn: []const u8) []const u8 {
    const slash = std.mem.lastIndexOfScalar(u8, arn, '/') orelse return arn;
    return arn[slash + 1 ..];
}

fn oidcToItem(allocator: std.mem.Allocator, p: Iam.OpenIDConnectProvider, cred_idx: usize) !ProviderItem {
    const item_name = try allocator.dupe(u8, parseName(p.arn));
...
fn samlToItem(allocator: std.mem.Allocator, p: Iam.SAMLProvider, cred_idx: usize) !ProviderItem {
    const item_name = try allocator.dupe(u8, parseName(p.arn));
    errdefer allocator.free(item_name);
    const arn = try allocator.dupe(u8, p.arn);
    errdefer allocator.free(arn);
    const account_id = try allocator.dupe(u8, parseAccountId(p.arn));
    errdefer allocator.free(account_id);
    const create_date = try allocator.dupe(u8, if (p.create_date.len >= 10) p.create_date[0..10] else p.create_date);
    errdefer allocator.free(create_date);
    const valid_until = try allocator.dupe(u8, if (p.valid_until.len >= 10) p.valid_until[0..10] else p.valid_until);
...
const FetchCtx = struct {
    ...
    items: std.ArrayList(ProviderItem) = .empty,
...
// Lists OIDC + SAML providers for a single profile (cred_idx into ctx.creds), appending into shared ctx.items.
fn listProfileThread(ctx: *FetchCtx, cred_idx: usize) void {
    ...
    const oidc_result = client.listOpenIDConnectProviders(.{}) catch |e| {
        setErr(ctx, e);
        return;
    };
    defer oidc_result.deinit();

    lockMutex(&ctx.mutex);
    for (oidc_result.providers) |p| {
        const item = oidcToItem(ctx.allocator, p, cred_idx) catch |e| {
            ctx.mutex.unlock();
            setErr(ctx, e);
            return;
        };
    ...
    const saml_result = client.listSAMLProviders(.{}) catch |e| {
        setErr(ctx, e);
        return;
    };
    defer saml_result.deinit();

    lockMutex(&ctx.mutex);
    for (saml_result.providers) |p| {
        const item = samlToItem(ctx.allocator, p, cred_idx) catch |e| {
            ctx.mutex.unlock();
            setErr(ctx, e);
            return;
        };
...
// OIDC providers carry no CreateDate from ListOpenIDConnectProviders; fetch it via GetOpenIDConnectProvider.
fn enrichWorker(ctx: *FetchCtx, total: usize) void {
    ...
    while (true) {
        ...
        lockMutex(&ctx.mutex);
        const is_oidc = ctx.items.items[i].provider_type == .oidc;
        const arn_copy = if (is_oidc) ctx.allocator.dupe(u8, ctx.items.items[i].arn) catch {
            ctx.mutex.unlock();
            continue;
        } else "";
        ...
        ctx.mutex.unlock();
        if (!is_oidc) continue;
        defer ctx.allocator.free(arn_copy);
        ...
        const gp = client.getOpenIDConnectProvider(.{ .open_id_connect_provider_arn = arn_copy }) catch continue;
        defer gp.deinit();

        const create_date = ctx.allocator.dupe(u8, if (gp.create_date.len >= 10) gp.create_date[0..10] else gp.create_date) catch continue;

        lockMutex(&ctx.mutex);
        if (ctx.items.items[i].create_date) |d| ctx.allocator.free(d);
        ctx.items.items[i].create_date = create_date;
        ctx.mutex.unlock();
...
fn fetchThread(ctx: *FetchCtx) void {
    ...
    // Phase 2: enrich OIDC providers with GetOpenIDConnectProvider in parallel, across all profiles' items.
    const total = blk: {
        lockMutex(&ctx.mutex);
...
sort_keys: [4]ProviderSortKey = .{ .name, undefined, undefined, undefined },
...
pub fn init(
    ...
    const bc = std.fmt.bufPrint(&view.breadcrumb_buf, "{s} {s} Identity Providers", .{ parent_breadcrumb, constants.SEP_ARROW }) catch view.breadcrumb_buf[0..0];
    view.breadcrumb_len = bc.len;
    return view;
}

fn freeState(self: *IamIdentityProvidersView) void {
    switch (self.state) {
        .loading, .ready => |ctx| {
...
fn refresh(self: *IamIdentityProvidersView) !void {
    self.freeState();
...
pub fn breadcrumb(self: *IamIdentityProvidersView) []const u8 {
    return self.breadcrumb_buf[0..self.breadcrumb_len];
}

pub fn deinit(self: *IamIdentityProvidersView) void {
...
fn effectiveFilter(self: *const IamIdentityProvidersView) []const u8 {
    if (self.live_filter.len > 0) return self.live_filter;
    return self.committed_filter orelse "";
}

fn matchesProvider(self: *const IamIdentityProvidersView, item: ProviderItem, text_f: []const u8) bool {
    if (!filter_mod.matchesText(item.name, text_f)) return false;
    if (self.filter_expr) |*fe| {
        const resolver = ProviderResolver{ .item = item };
        if (!filter_mod.evalExpr(fe.expr, resolver)) return false;
    }
    return true;
}

fn visibleCount(self: *const IamIdentityProvidersView, items: []const ProviderItem, text_f: []const u8) usize {
    var n: usize = 0;
    for (items) |item| {
        if (self.matchesProvider(item, text_f)) n += 1;
    }
    return n;
}

fn recomputeSort(self: *IamIdentityProvidersView, items: []const ProviderItem) void {
...
fn ensureSorted(self: *IamIdentityProvidersView, items: []const ProviderItem) void {
    if (!self.sort_dirty and self.sorted_indices.len == items.len) return;
    self.recomputeSort(items);
    self.sort_dirty = false;
}

pub fn setSort(self: *IamIdentityProvidersView, keys: []const ProviderSortKey, dir: constants.SortDir) void {
...
pub fn clearSort(self: *IamIdentityProvidersView) void {
...
pub fn setLiveFilter(self: *IamIdentityProvidersView, text: []const u8) void {
...
pub fn commitFilter(self: *IamIdentityProvidersView, text: []const u8) void {
    if (self.committed_filter) |f| self.alloc.free(f);
...
pub fn setFilterExpr(self: *IamIdentityProvidersView, result: filter_mod.ParseResult) void {
    if (self.filter_expr) |*fe| fe.deinit();
...
pub fn clearFilterExpr(self: *IamIdentityProvidersView) void {
    if (self.filter_expr) |*fe| fe.deinit();
...
pub fn handleEvent(self: *IamIdentityProvidersView, event: Event, vctx: ViewContext) !Action {
    ...
    switch (event) {
        .key => |k| switch (k) {
            ...
            .enter => {
                ...
                var vis_idx: usize = 0;
                var found: ?ProviderItem = null;
                for (self.sorted_indices) |orig_idx| {
                    const item = ctx.items.items[orig_idx];
                    if (!self.matchesProvider(item, filter)) continue;
                ...
                const provider_name = vctx.allocator.dupe(u8, item.name) catch {
                    ctx.mutex.unlock();
                    return .none;
                };
                const arn = vctx.allocator.dupe(u8, item.arn) catch {
                    vctx.allocator.free(provider_name);
                    ctx.mutex.unlock();
                    return .none;
                };
                const account_id = vctx.allocator.dupe(u8, item.account_id) catch {
                    vctx.allocator.free(provider_name);
                ...
                ctx.mutex.unlock();
                defer vctx.allocator.free(provider_name);
                defer vctx.allocator.free(arn);
                defer vctx.allocator.free(account_id);

                switch (provider_type) {
                    .oidc => {
                        const detail = try IamOidcProviderView.init(
                            vctx.allocator,
                            vctx.io,
                            ctx.creds[cred_idx],
                            provider_name,
                        ...
                        return .{ .push = .{ .iam_oidc_provider = detail } };
                    },
                    .saml => {
                        const detail = try IamSamlProviderView.init(
                            vctx.allocator,
                            vctx.io,
                            ctx.creds[cred_idx],
                            provider_name,
                        ...
                        return .{ .push = .{ .iam_saml_provider = detail } };
                    },
                }
            },
            .escape => {
...
fn nameWidth(inner: usize) usize {
    const dividers = 4; // between NAME|TYPE, TYPE|ACCOUNT, ACCOUNT|CREATED, CREATED|EXPIRES
    const others = TYPE_W + ACCOUNT_W + CREATED_W + EXPIRES_W + dividers;
...
fn writeVert(self: *IamIdentityProvidersView, writer: *std.Io.Writer, selected: bool, reset: bool) !void {
    if (selected) {
...
fn writeSepRow(self: *IamIdentityProvidersView, writer: *std.Io.Writer, name_w: usize, bottom: bool) !void {
    ...
    try writer.writeAll(mid);
    for (0..EXPIRES_W) |_| try writer.writeAll(constants.HORIZONTAL);
    try writer.writeAll(right);
    try writer.writeAll(terminal.RESET);
}

fn writeHeaderCell(self: *IamIdentityProvidersView, writer: *std.Io.Writer, text: []const u8, cell_w: usize) !void {
    const content_w = if (cell_w >= 2) cell_w - 2 else 0;
...
fn writeHeaderRow(self: *IamIdentityProvidersView, writer: *std.Io.Writer, name_w: usize) !void {
    try writer.writeAll(self.fg_color);
    try writer.writeAll(constants.VERTICAL);
    try writer.writeAll(terminal.RESET);
    ...
    try writer.writeAll(constants.VERTICAL);
    try writer.writeAll(terminal.RESET);
    try self.writeHeaderCell(writer, "EXPIRES", EXPIRES_W);
    try writer.writeAll(self.fg_color);
    try writer.writeAll(constants.VERTICAL);
    try writer.writeAll(terminal.RESET);
}

fn writeProviderRow(self: *IamIdentityProvidersView, writer: *std.Io.Writer, item: ProviderItem, loading: bool, sel: bool, name_w: usize) !void {
    ...
    try self.writeVert(writer, sel, !sel);
    try writePaddedCell(writer, typeLabel(item.provider_type), TYPE_W);

    try self.writeVert(writer, sel, !sel);
    try writePaddedCell(writer, item.account_id, ACCOUNT_W);

    const created_placeholder = if (loading and item.provider_type == .oidc) "…" else "-";
    try self.writeVert(writer, sel, !sel);
    try writePaddedCell(writer, item.create_date orelse created_placeholder, CREATED_W);

    try self.writeVert(writer, sel, !sel);
    try writePaddedCell(writer, item.valid_until orelse "-", EXPIRES_W);

    try self.writeVert(writer, sel, true);
}

fn writeEmptyRow(self: *IamIdentityProvidersView, writer: *std.Io.Writer, name_w: usize) !void {
    ...
    try self.writeVert(writer, false, true);
    for (0..EXPIRES_W) |_| try writer.writeByte(' ');
    try self.writeVert(writer, false, true);
}

pub fn render(self: *IamIdentityProvidersView, writer: *std.Io.Writer, size: Coord) !void {
        ...
        .loading, .ready => |ctx| {
            ...
            for (self.sorted_indices) |orig_idx| {
                ...
                if (vis_idx >= self.scroll_offset and rendered < data_rows) {
                    try self.writeProviderRow(writer, item, loading, vis_idx == self.selected, name_w);
                    try writer.writeAll("\r\n");
                    rendered += 1;
                }
                vis_idx += 1;
        ...
        .failed => |e| {
            for (0..data_rows) |row| {
                ...
                if (row == 0) {
                    ...
                    const msg = std.fmt.bufPrint(&buf, " Error: {s}", .{@errorName(e)}) catch " Error loading identity providers";
