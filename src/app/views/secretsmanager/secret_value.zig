...
const SecretValueView = @This();
pub const name: []const u8 = "Secret Value";

const MASK = "•••••••••••• (press Enter to reveal)";

// ─── Background GetSecretValue context ───────────────────────────────────────
...
const FetchCtx = struct {
    ...
    secret_id: []u8,
    region: []u8,
    /// Pretty-printed (indent_2) JSON secret string, falling back to the raw
    /// value verbatim if it doesn't parse as JSON. Null if the secret has a
    /// binary value instead of a string value.
    value: ?[]u8 = null,
    err: ?anyerror = null,
...
fn fetchThread(ctx: *FetchCtx) void {
    ...
    const result = client.getSecretValue(.{ .secret_id = ctx.secret_id }) catch |e| {
        ctx.err = e;
        return;
    };
    defer result.deinit();

    if (result.secret_string) |s| {
        ctx.value = prettyPrintJson(ctx.allocator, s) catch |e| {
            ctx.err = e;
            return;
        };
    } else {
        ctx.value = ctx.allocator.dupe(u8, "(binary secret value — not displayable)") catch |e| {
            ctx.err = e;
            return;
        };
    }
...
pub fn init(
    ...
    const bc = std.fmt.bufPrint(&view.breadcrumb_buf, "{s} {s} Value", .{ parent_breadcrumb, constants.SEP_ARROW }) catch view.breadcrumb_buf[0..0];
    view.breadcrumb_len = bc.len;
    return view;
}

pub fn breadcrumb(self: *SecretValueView) []const u8 {
    return self.breadcrumb_buf[0..self.breadcrumb_len];
}

pub fn deinit(self: *SecretValueView) void {
    const alloc = self.ctx.allocator;
    if (!self.ctx.done.load(.acquire)) self.ctx.thread.join();
    if (self.ctx.value) |v| alloc.free(v);
...
fn refresh(self: *SecretValueView) !void {
...
pub fn handleEvent(self: *SecretValueView, event: Event, _: ViewContext) !Action {
    switch (event) {
        .key => |k| switch (k) {
            .ctrl_c => return .quit,
            .char => |c| switch (c) {
                ...
                'k' => if (self.revealed and self.scroll > 0) {
                    self.scroll -= 1;
                },
                'g' => {
                    if (self.pending_g) {
                ...
                'G' => {
                    self.pending_g = false;
                    if (self.revealed) self.scroll = std.math.maxInt(usize) / 2;
                },
                else => {
                    self.pending_g = false;
                },
            },
            .down => if (self.revealed) {
                self.scroll += 1;
            },
            .up => if (self.revealed and self.scroll > 0) {
                self.scroll -= 1;
            },
            .enter => self.revealed = !self.revealed,
            .escape => return .pop,
            else => {},
        },
        else => {},
...
pub fn render(self: *SecretValueView, writer: *std.Io.Writer, size: Coord) !void {
    ...
    if (!self.revealed) {
        try self.writeStatus(writer, inner_w, data_rows, MASK);
        return;
    }

    const value = self.ctx.value orelse "";
    const width_changed = self.last_lines_width != inner_w;
    if (self.lines == null or width_changed) {
        if (self.lines) |l| self.alloc.free(l);
        self.lines = try computeLines(self.alloc, value, inner_w);
...
fn writeBottom(self: *SecretValueView, writer: *std.Io.Writer, inner_w: usize) !void {
    try writer.writeAll(self.fg_color);
...
test "prettyPrintJson formats compact document" {
    ...
    const pretty = try prettyPrintJson(allocator, "{\"user\":\"admin\",\"password\":\"hunter2\"}");
    defer allocator.free(pretty);
    try std.testing.expect(std.mem.indexOf(u8, pretty, "\n") != null);
    try std.testing.expect(std.mem.indexOf(u8, pretty, "\"user\": \"admin\"") != null);
}

test "prettyPrintJson falls back on plain text" {
    const allocator = std.testing.allocator;
    const pretty = try prettyPrintJson(allocator, "plain-secret-value");
    defer allocator.free(pretty);
    try std.testing.expectEqualStrings("plain-secret-value", pretty);
}
