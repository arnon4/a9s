...
pub const IamListAttachedUserPoliciesError = error{
...
pub fn listAttachedUserPolicies(client: anytype, options: Options) !Result {
    ...
    if (result.status != .ok) {
        const code = xml.extractTagContent(client.allocator, resp_body, "Code") catch {
            std.log.err("IAM ListAttachedUserPolicies error: status {d}", .{@intFromEnum(result.status)});
            return aws_errors.fromStatus(result.status);
        };
        defer client.allocator.free(code);
        std.log.err("IAM ListAttachedUserPolicies error: {s} (status {d})", .{ code, @intFromEnum(result.status) });
        inline for (@typeInfo(IamListAttachedUserPoliciesError).error_set.?) |entry| {
            if (std.mem.eql(u8, entry.name, code)) return @field(IamListAttachedUserPoliciesError, entry.name);
        }
...
fn buildBody(allocator: Allocator, user_name: []const u8, params: Params) ![]u8 {
    ...
    try body.appendSlice(allocator, "Action=ListAttachedUserPolicies&Version=2010-05-08&UserName=");
...
test "buildBody user name only" {
    ...
    try std.testing.expectEqualStrings("Action=ListAttachedUserPolicies&Version=2010-05-08&UserName=alice", body);
}

test "buildBody with marker" {
    const body = try buildBody(std.testing.allocator, "alice", .{ .marker = "abc" });
    defer std.testing.allocator.free(body);
    try std.testing.expectEqualStrings("Action=ListAttachedUserPolicies&Version=2010-05-08&UserName=alice&Marker=abc", body);
}
