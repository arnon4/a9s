...
pub const IamListAttachedRolePoliciesError = error{
...
pub const AttachedPolicy = struct {
    ...
    pub fn clone(self: AttachedPolicy, allocator: Allocator) !AttachedPolicy {
        const policy_name = try allocator.dupe(u8, self.policy_name);
        errdefer allocator.free(policy_name);
        const policy_arn = try allocator.dupe(u8, self.policy_arn);
...
pub fn listAttachedRolePolicies(client: anytype, options: Options) !Result {
    const body = try buildBody(client.allocator, options.role_name, options.params);
    ...
    if (result.status != .ok) {
        const code = xml.extractTagContent(client.allocator, resp_body, "Code") catch {
            std.log.err("IAM ListAttachedRolePolicies error: status {d}", .{@intFromEnum(result.status)});
            return aws_errors.fromStatus(result.status);
        };
        defer client.allocator.free(code);
        std.log.err("IAM ListAttachedRolePolicies error: {s} (status {d})", .{ code, @intFromEnum(result.status) });
        inline for (@typeInfo(IamListAttachedRolePoliciesError).error_set.?) |entry| {
            if (std.mem.eql(u8, entry.name, code)) return @field(IamListAttachedRolePoliciesError, entry.name);
        }
...
fn buildBody(allocator: Allocator, role_name: []const u8, params: Params) ![]u8 {
    var body: std.ArrayList(u8) = .empty;
    errdefer body.deinit(allocator);

    try body.appendSlice(allocator, "Action=ListAttachedRolePolicies&Version=2010-05-08&RoleName=");
    const role_encoded = try uriEncode(allocator, role_name);
    defer allocator.free(role_encoded);
    try body.appendSlice(allocator, role_encoded);
...
test "buildBody role name only" {
    const body = try buildBody(std.testing.allocator, "my-role", .{});
    defer std.testing.allocator.free(body);
    try std.testing.expectEqualStrings("Action=ListAttachedRolePolicies&Version=2010-05-08&RoleName=my-role", body);
}

test "buildBody with marker" {
    const body = try buildBody(std.testing.allocator, "my-role", .{ .marker = "abc" });
    defer std.testing.allocator.free(body);
    try std.testing.expectEqualStrings("Action=ListAttachedRolePolicies&Version=2010-05-08&RoleName=my-role&Marker=abc", body);
}
